...
const InputModalForm = ({
  ...
  onSubmit,
  onCancel,
}: Omit<InputModalProps, "isOpen">): JSX.Element => {
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-5">
            ...
            <input
              ...
              className="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:border-blue-500/50 focus:outline-hidden focus:ring-1 focus:ring-blue-500/50"
            />
          </div>
          <div className="flex justify-end gap-3 p-4 border-t border-slate-800 bg-slate-900/50 rounded-b-xl">
            ...
            <button
              type="submit"
              disabled={!value.trim()}
              className="px-3 py-1.5 rounded-md bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium shadow-lg shadow-blue-500/20 transition disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-blue-500/50"
            >
              {submitLabel}
            </button>
          </div>
...
export const InputModal = ({ isOpen, ...props }: InputModalProps): JSX.Element | null => {
  return isOpen ? <InputModalForm key={props.defaultValue} {...props} /> : null;
};
